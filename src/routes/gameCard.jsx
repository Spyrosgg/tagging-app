...
import { winterData, summerData } from "../lib/chrs.coordinates";
...
function GameCard({ inGame, setInGame}) {
  ...
  const currentScore = useRef("loading");
  ...
  async function setEnd() {
    ...
    currentScore.current = scoreTemp;
    console.log("score", currentScore.current);
    
  }
  
  function handleCheck(chr) {
    const seasonData = (state.season === "winter" ? winterData : summerData)
    seasonData.map((who) => {
  ...
  return (
    <>
      <div>
        ...
        {won && <FireworksComp time={currentScore.current} currentID={currentID.current} />}
        {/* <StopWatch /> */}
        {/* <FireworksComp currentID={currentID} /> */}
        <img
          src={state.season === "winter" ? winter : summer}
          alt="game card"
